...
| [**matlab-sim**](matlab/SKILL.md) | Numerical / scripting + Simulink | One-shot `sim run --solver matlab` | v0 🟡 | `.m` scripts one-shot + Simulink `.slx`/`.mdl` one-shot dispatch via `sim_shim.run` (Parquet/MAT result pointer); persistent session planned for v1 |
...
- **2026-04-26** 🪪 **Anthropic skill format validated end-to-end via Claude Code in sim-benchmark.** Demonstrated that `<solver>/SKILL.md` files are picked up natively by Claude Code's skill loader when the directory is symlinked into `~/.claude/skills/` — no extra runtime layer, no plugin marketplace, just the YAML frontmatter (`name` + `description`) doing what it was designed to do. Reference implementation: [sim-benchmark `tools/claude_code_cn.py:_register_sim_skills`](https://github.com/svd-ai-lab/sim-benchmark/blob/main/tools/claude_code_cn.py) auto-registers `sim-cli` (mandatory) plus one entry per ok solver from `sim --json check`, mapping driver names to skill dirs (`ls_dyna → lsdyna`, etc.). Inside the trial container, Claude Code's init event then reports `"skills":[…,"calculix","sim-cli","openfoam"]` and the agent invokes them via `Skill: openfoam` — no more `ls $SIM_SKILLS_ROOT` thrash, no fall-back to bare-bash workflows. Downstream impact: sim-benchmark v13 on `cases/neutral/` (Kimi-K2.5, n=3) jumped from mean 0.62 → 0.83 with **no changes to instructions, scoring, or skill content** — the +0.21 is purely from the agent finally seeing the skills it was supposed to all along.
- **2026-04-25** 🧱 **Simulink coverage in `matlab-sim`** (Phase E of [sim-cli#27](https://github.com/svd-ai-lab/sim-cli/issues/27)) — the MATLAB skill now documents the Phase A + minimum-viable Phase B Simulink surface. `sim run <model.slx|mdl> --solver matlab` dispatches through a `sim_shim.run` package helper (`addpath(resources) → load_system → onCleanup(close_system) → sim_shim.run(model, '{}', .sim/<model>/)`) which runs `sim()`, flattens `Simulink.SimulationOutput` to a timetable, and emits a `{ok, result_file, format, signals}` JSON pointer — Parquet preferred, MAT fallback. `sim check matlab` gains a per-install `simulink: installed | not found on disk` line driven by a filesystem probe of `toolbox/simulink/simulink/`. New reference doc: [`matlab/base/reference/simulink.md`](matlab/base/reference/simulink.md) — full `sim_shim.run(model, params_json, out_dir)` contract, Python-side reading recipes for both formats, and an explicit list of what is **not** wired yet (rest of `+sim_shim/` including `parsim` sweeps — Phase B; `models`/`blocks`/`signals`/`figures.summary` inspect verbs — Phase C; typed `SimulationResult`/`SweepResult` — Phase D; sample `.slx` fixtures in sim-datasets — Phase F; shared/persistent Simulink sessions — listed under Non-goals). Frontmatter + README grid row updated; no new top-level skill folder was added — scope (one helper, one suffix branch) was too narrow for a sibling `simulink/` to justify.
- **2026-04-23** 🖱 **`sim-cli/gui/` skill — runtime-injected GUI actuation for agents**. New skill at [`sim-cli/gui/SKILL.md`](sim-cli/gui/SKILL.md) documents the `gui` object sim-cli injects into `sim exec` whenever the active driver is running in GUI mode. Full API (`find` / `click` / `send_text` / `close` / `activate` / `screenshot` / `list_windows` / `snapshot`), the four typical patterns (dismiss login dialog, confirm overwrite, walk UIA tree when labels unknown, per-window screenshot), and pitfall table. Ships three copy-ready snippets: [`dismiss_login_dialog.py`](sim-cli/gui/snippets/dismiss_login_dialog.py) (COMSOL Cortex 连接到 / English Connect to COMSOL), [`fill_file_save_dialog.py`](sim-cli/gui/snippets/fill_file_save_dialog.py) (Fluent *File > Write > Case*), [`dismiss_script_error.py`](sim-cli/gui/snippets/dismiss_script_error.py) (Mechanical IronPython error modal). Each per-solver `SKILL.md` ([`fluent`](fluent/SKILL.md), [`comsol`](comsol/SKILL.md), [`flotherm`](flotherm/SKILL.md)) gets a short **"GUI actuation"** section pointing at the shared reference and enumerating the dialogs that solver is known to pop — the API itself is not duplicated per solver. New convention in [`CLAUDE.md`](CLAUDE.md): runtime-injected tool skills live at `sim-cli/<tool>/` (peer of `sim-cli/SKILL.md` + `sim-cli/reference/`), not as top-level peers of solvers.
