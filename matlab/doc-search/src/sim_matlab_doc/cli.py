"""`sim-matlab-doc` CLI: search, retrieve, where. No index step — scans on demand."""
...
def _cmd_search(args: argparse.Namespace) -> int:
    doc_root = _resolve_root(args.matlab_root)
...
def _cmd_where(args: argparse.Namespace) -> int:
    print(_resolve_root(args.matlab_root))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="sim-matlab-doc",
        description="On-demand search over an installed MATLAB documentation tree.",
    )
    p.add_argument("--version", action="version", version=f"sim-matlab-doc {__version__}")
    ...
    def add_root(sp: argparse.ArgumentParser) -> None:
        sp.add_argument(
            ...
            type=Path,
            default=None,
            help="Override the matlabroot install dir or help/ doc tree.",
        )
    ...
    ps.add_argument("--module", "-m", default=None,
                    help="Substring filter on toolbox folder name, e.g. 'matlab', 'optim', 'simulink'.")
    ...
    pr.add_argument("rel_path", help="Toolbox-relative path returned by `search`.")
    add_root(pr)
    pr.set_defaults(func=_cmd_retrieve)

    pw = sub.add_parser("where", help="Print the detected MATLAB doc root and exit.")
    add_root(pw)
