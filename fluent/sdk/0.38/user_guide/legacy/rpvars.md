...
You can create custom user-defined rpvars with specific types using the `create` method. The `var_type` parameter accepts either Python types ( `int` , `float` , `bool` , `str` ) or `RPVarType` enum values. Use `None` to create a custom-typed rpvars that can hold any value type.
...
When creating typed rpvars, the initial value must match the specified type. Type-checking is enforced to prevent mismatches:
...
Note: Custom-typed rpvars (created with `var_type=None` ) do not enforce type restrictions and can be reassigned to any value type.
