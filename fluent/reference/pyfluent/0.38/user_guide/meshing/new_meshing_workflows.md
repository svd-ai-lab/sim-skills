...
Use `create_workflow()` to build a custom workflow. The example below demonstrates how to create and populate a workflow.
...
Tasks can be deleted individually or in groups. In Enhanced Meshing Workflow, pass task objects to `list_of_tasks` :

Note

Behavior change. Delete-by-name (strings) is replaced by passing task objects. Calling `task.delete()` still works. See Legacy meshing interface for earlier usage.
