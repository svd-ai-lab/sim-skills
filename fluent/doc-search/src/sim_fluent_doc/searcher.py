"""Grep-style search over the Ansys Fluent help tree, no index."""
...
# A topic folder is any direct child of the help root whose name starts
# with one of these prefixes. Keeping this permissive future-proofs the
# code against vendor additions.
_TOPIC_PREFIXES = ("flu_", "pyfluent", "wb2_")
...
def _collect_html_paths(doc_root: Path, module: str | None) -> list[Path]:
    """Enumerate all .html files under the Fluent topic folders.

    A "topic folder" is any direct child directory of `doc_root` whose
    name starts with `flu_`, `pyfluent`, or `wb2_`. When `module` is
    given, it's applied as a case-sensitive substring filter on the
    folder name (e.g. `flu_ug`, `flu_udf`, `pyfluent`).
    """
    ...
    for child in doc_root.iterdir():
        if not child.is_dir():
            continue
        name = child.name
        if not any(name.startswith(p) for p in _TOPIC_PREFIXES):
            continue
        if module and module not in name:
            continue
        topic_dirs.append(child)
    return [f for d in topic_dirs for f in d.rglob("*.html")]
...
def search(
    ...
    """Scan every HTML file under matching topic folders, return ranked hits.
