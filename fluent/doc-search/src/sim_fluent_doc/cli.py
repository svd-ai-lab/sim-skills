"""`sim-fluent-doc` CLI: search, retrieve, where. No index step — scans on demand."""
...
def _cmd_search(args: argparse.Namespace) -> int:
    doc_root = _resolve_root(args.ansys_root)
...
def _cmd_where(args: argparse.Namespace) -> int:
    print(_resolve_root(args.ansys_root))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="sim-fluent-doc",
        description="On-demand search over an installed Ansys Fluent documentation tree.",
    )
    p.add_argument("--version", action="version", version=f"sim-fluent-doc {__version__}")
    ...
    def add_root(sp: argparse.ArgumentParser) -> None:
        sp.add_argument(
            ...
            type=Path,
            default=None,
            help="Override the Ansys install dir (e.g. 'C:\\Program Files\\ANSYS Inc\\v252') "
            "or the help dir itself.",
        )
    ...
    ps.add_argument("--module", "-m", default=None,
                    help="Substring filter on topic-folder name, e.g. 'flu_ug', 'flu_udf', "
                         "'pyfluent'.")
    ...
    pr.add_argument("rel_path", help="Help-root-relative path returned by `search`.")
    add_root(pr)
    pr.set_defaults(func=_cmd_retrieve)

    pw = sub.add_parser("where", help="Print the detected Fluent doc root and exit.")
    add_root(pw)
